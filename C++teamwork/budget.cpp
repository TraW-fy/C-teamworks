...
void checkBudgetAlert(const vector<Record>& records, const Budget& budget, const string& month) {
    ...
    if (totalExpense >= budget.totalBudget * 0.8) {
        cout << "🆘  警告：当月总支出已达到总预算的 " << fixed << setprecision(1) << (totalExpense / budget.totalBudget * 100) << "%！" << endl;
    }
    
    // 检查各分类预算
    if (budget.foodBudget > 0 && foodAmount >= budget.foodBudget * 0.8) {
        cout << "🍜  警告：餐饮支出已达到预算的 " << fixed << setprecision(1) << (foodAmount / budget.foodBudget * 100) << "%！" << endl;
    }
    
    if (budget.studyBudget > 0 && studyAmount >= budget.studyBudget * 0.8) {
        cout << "📚  警告：学习支出已达到预算的 " << fixed << setprecision(1) << (studyAmount / budget.studyBudget * 100) << "%！" << endl;
    }
    
    if (budget.entertainmentBudget > 0 && entertainmentAmount >= budget.entertainmentBudget * 0.8) {
        cout << "🎮  警告：娱乐支出已达到预算的 " << fixed << setprecision(1) << (entertainmentAmount / budget.entertainmentBudget * 100) << "%！" << endl;
    }
    
    if (budget.transportBudget > 0 && transportAmount >= budget.transportBudget * 0.8) {
        cout << "🚗  警告：交通支出已达到预算的 " << fixed << setprecision(1) << (transportAmount / budget.transportBudget * 100) << "%！" << endl;
    }
